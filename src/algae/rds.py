...
def is_cluster_renaming(cluster_identifier: str) -> True:
    try:
        ...
        _logger.warning("cluster not found, this is the side effect of "
                        "renaming")
        return True
...
def create_cluster_db_instances(cluster_identifier: str):
    ...
    _logger.info(f"creating cluster database instance in cluster "
    ...
    status_code = response['ResponseMetadata']['HTTPStatusCode']
    if status_code != 200:
        _logger.error(f"failed to create db instance {cluster_identifier} with "
                      f"status code {status_code}")
        raise Exception(f"failed to modify {cluster_identifier} with status "
                        f"code {status_code}")
...
def upgrade_clone_cluster(cluster_identifier: str, engine_version: str):
    _logger.info(f"modifying cluster clone \"{cluster_identifier}\" to engine "
    ...
    status_code = response['ResponseMetadata']['HTTPStatusCode']
    if status_code != 200:
        _logger.error(f"failed to modify {cluster_identifier} with status code "
                      f"{status_code}")
        raise Exception(f"failed to modify {cluster_identifier} with status "
                        f"code {status_code}")

    _logger.info(f"modified clone cluster \"{cluster_identifier}\" with engine "
                 f"version \"{engine_version}\"")

    polling2.poll(
        lambda: is_cluster_upgrading(cluster_identifier),
        step=60,
        poll_forever=True
    )
...
def upgrade_clone_cluster_identifier(cluster_identifier: str,
    ...
    _logger.info(f"modifying cluster id \"{cluster_identifier}\" name to \""
    ...
    polling2.poll(
        lambda: is_cluster_renaming(cluster_identifier),
        step=60,
        poll_forever=True
    )

    polling2.poll(
        lambda: is_cluster_available(new_cluster_identifier),
        step=60,
        poll_forever=True
    )
